...
public class MiniProfiler : EditorWindow
{
    ...
    void AnalyzeLevel()
    {
        ...
        foreach (MeshFilter mf in meshFilters)
        {
            ...
            if(!willBeCombined)
            {
                // TODO: Figure out which meshes shouldn't combine with which components
        ...
        if (nonCombinedMeshCount > 50)
        {
            m_SuggestionStrings.Add(nonCombinedMeshCount + " meshes in the scene are not setup to be combined on game start. Make sure that all the meshes " +
                "that will never move, change, or be removed during play are included inside the \"Combine Parents\" list of the \"Mesh Combiner\" script in the scene, so they can be combined for greater performance. \n \n" + 
